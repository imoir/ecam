...
/* --------------------------------------- */
#define CrashBufferSize 1024
__NOINIT_ATTR static int crashBufferIndex;
__NOINIT_ATTR static char crashBuffer[CrashBufferSize];

void panic_print_char_buff(const char c)
{
    if (crashBufferIndex < CrashBufferSize)
    {
        crashBuffer[crashBufferIndex] = c;
        crashBufferIndex++;
    }
}
/* --------------------------------------- */
...
void app_main()
{
    ...
    printf("Otherwise, a picture will be taken every 30 seconds ...\n");
    ...
    while (true)
    {
        ...
        printf(" ... task to take picture launched \n");
